...
fs=128;
T=1/fs;
%% Setup: DMD Heatmaps for DEAP Dataset, exports dmd heatmap and tabular version for RF, windowing to smaller sample
...
for subject = 1:32
    ...
    data=reshape(data,[40/(window_size/63),40,8064*(window_size/63)]);
    ...
    for trial = 1:size(data,1)
        ...
        r = 60; % number of modes, remember DMD generates complex conjugates
...
%         close all
        ...
        filename2=['export/DMD_window/1second/S' num2str(subject),'T' num2str(trial), extension]
        saveas(gcf,filename2)
        close all
        ...
        export_vec=[export_vec;[subject,trial,reshape([real(Phi_phys_unique)',imag(Phi_phys_unique)',fn_count',zeta_count'],1,[])]];
        %export_comps=[export_comps;[subject_exp,trial_exp,Phi_phys_exp_re,Phi_phys_exp_im,chann_exp,mode_exp,fn_exp,zeta_exp]];

    end
end
writematrix(export_vec,'dmd_deap_100modes_1window_nohead.csv')
...
writecell(out,'dmd_deap_100modes_1window.csv')
