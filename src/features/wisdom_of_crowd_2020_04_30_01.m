...
%     close all
    ...
    A_n4_herm=0.5*(A_n4_average+A_n4_average');
    A_n4_skew=0.5*(A_n4_average-A_n4_average');
    n4_herm_ratio=norm(A_n4_skew)/norm(A_n4_herm);
    ...
    [fn_n4_3, zeta_n4_3, Phi_n4_3]=modalparams(A_n4_herm,eye(12),T);
    Phi_n4_2=Phi_n4_2{1};
    ...
    A_NeXT_herm=0.5*(A_NeXT_average+A_NeXT_average');
    A_NeXT_skew=0.5*(A_NeXT_average-A_NeXT_average');
    NeXT_herm_ratio=norm(A_NeXT_skew)/norm(A_NeXT_herm);
    ...
    [fn_NeXT_3, zeta_NeXT_3, Phi_NeXT_3]=modalparams(A_NeXT_herm,eye(12),T);
    ...
    A_cov_herm=0.5*(A_cov_average+A_cov_average');
    A_cov_skew=0.5*(A_cov_average-A_cov_average');
    cov_herm_ratio=norm(A_cov_skew)/norm(A_cov_herm);
    ...
    [fn_cov_3, zeta_cov_3, Phi_cov_3]=modalparams(A_cov_herm,eye(12),T);
    ...
    A_data_herm=0.5*(A_data_average+A_data_average');
    A_data_skew=0.5*(A_data_average-A_data_average');
    data_herm_ratio=norm(A_data_skew)/norm(A_data_herm);
    ...
    [fn_data_3, zeta_data_3, Phi_data_3]=modalparams(A_data_herm,eye(12),T); % eye(12) for now, sad face
...
%     close all
    ...
    norm_data_theta=max(abs(theta_data_mean), [],'all');
    theta_data_mean_norm=theta_data_mean/norm_data_theta;
    
    norm_n4_theta=max(abs(theta_n4_mean), [],'all');
    theta_n4_mean_norm=theta_n4_mean/norm_n4_theta;
    
    norm_NeXT_theta=max(abs(theta_NeXT_mean), [],'all');
    theta_NeXT_mean_norm=theta_NeXT_mean/norm_NeXT_theta;
